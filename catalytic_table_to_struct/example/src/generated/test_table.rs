...
pub const INSERT_USING_QUERY : & str = "insert into test_table(b, c, d, a, e) values (?, ?, ?, ?, ?) using ttl ? and timestamp ? and timeout ?" ;
...
impl<'a> TestTableRef<'a> {
    ...
    pub fn insert_using_qv(&self, params: UsingParams) -> Result<Insert, SerializeValuesError> {
        let mut serialized = SerializedValues::with_capacity(8usize);
        serialized.add_value(&self.b)?;
        serialized.add_value(&self.c)?;
        serialized.add_value(&self.d)?;
        serialized.add_value(&self.a)?;
